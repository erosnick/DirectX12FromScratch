// 3-PlacedTexture.cpp : Defines the entry point for the application.
//

#include "framework.h"
#include "3-PlacedTexture.h"
...
DXGI_FORMAT LoadTexture2D(ComPtr<ID3D12Device4>& pID3DDevice, const WCHAR* pszTexcuteFileName, ComPtr<IWICBitmapSource>& pIBMP, DXGI_FORMAT& stTextureFormat, UINT& nTextureW, UINT& nTextureH, UINT& nBPP, UINT& nPicRowPitch, ComPtr<ID3D12Heap>& pITextureHeap, ComPtr<ID3D12Resource>& pITexture, ComPtr<ID3D12Heap>& pIUploadHeap, ComPtr<ID3D12CommandQueue>& pICommandQueue, ComPtr<ID3D12GraphicsCommandList>& pICommandList, ComPtr<ID3D12CommandAllocator> &pICommandAllocator, ComPtr<ID3D12PipelineState>& pIPipelineState, ComPtr<ID3D12Fence>& pIFence)
{
	//---------------------------------------------------------------------------------------------
	// 16、使用WIC创建并加载一个2D纹理
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	ComPtr<ID3D12Heap>					pIUploadHeapShare;
	...
	try
	{
		...
		LoadTexture2D(pID3DDevice, _T("Texture/bear_flipX.jpg"), pIBMP, stTextureFormat, nTextureW, nTextureH, nBPP, nPicRowPitch, pITextureHeap, pITexcute1, pIUploadHeap, pICommandQueue, pICommandList, pICommandAllocator, pIPipelineState, pIFence);

		auto textureFormat = LoadTexture2D(pID3DDevice, _T("Texture/bear_flipX.jpg"), pIBMP, stTextureFormat, nTextureW, nTextureH, nBPP, nPicRowPitch, pITextureHeap, pITexcute2, pIUploadHeap, pICommandQueue, pICommandList, pICommandAllocator, pIPipelineState, pIFence);
		...
		stSRVDesc.Format = textureFormat;
