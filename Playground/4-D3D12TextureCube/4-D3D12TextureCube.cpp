// 3-D3D12TextureCube.cpp : Defines the entry point for the application.
//
...
#include "..\Engine\Source\D3D12\d3dx12.h"
...
ComPtr<ID3D12Resource> pITextureUpload1;
ComPtr<ID3D12Resource> pITextureUpload2;

UINT64 n64BufferOffset = 0;
UINT64 n64TextureHeapOffset = 0;
...
DXGI_FORMAT LoadTexture2D(ComPtr<ID3D12Device4>& pID3DDevice, const WCHAR* pszTexcuteFileName, ComPtr<IWICBitmapSource>& pIBMP, DXGI_FORMAT& stTextureFormat, UINT& nTextureW, UINT& nTextureH, UINT& nBPP, UINT& nPicRowPitch, ComPtr<ID3D12Heap>& pITextureHeap, ComPtr<ID3D12Resource>& pITexture, ComPtr<ID3D12Resource>& pITextureUpload , ComPtr<ID3D12Heap>& pIUploadHeap, ComPtr<ID3D12CommandQueue>& pICommandQueue, ComPtr<ID3D12GraphicsCommandList>& pICommandList, ComPtr<ID3D12CommandAllocator>& pICommandAllocator, ComPtr<ID3D12PipelineState>& pIPipelineState, ComPtr<ID3D12Fence>& pIFence)
{
	//---------------------------------------------------------------------------------------------
	// 16、使用WIC创建并加载一个2D纹理
	...
	GRS_THROW_IF_FAILED(pID3DDevice->CreatePlacedResource(
		pITextureHeap.Get()
		, GRS_UPPER(n64TextureHeapOffset, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT)
	...
	n64TextureHeapOffset += GRS_UPPER(n64UploadBufferSize, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	UINT nTextureH = 0u;
	UINT nBPP = 0u;
	DXGI_FORMAT stTextureFormat = DXGI_FORMAT_R8G8B8A8_UNORM;
	...
	ComPtr<ID3D12Resource>				pITexcute1;
	ComPtr<ID3D12Resource>				pITexcute2;
	...
	try
	{
		...
		stSRVHeapDesc.NumDescriptors = 3;	// 2 SRVs + 1 CBV
		...
		stDSPRanges[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 0, 0);
		...
		pID3DDevice->CreateDescriptorHeap(&samplerHeapDesc, IID_PPV_ARGS(&samplerHeap));
		...
		auto samplerHandle = samplerHeap->GetCPUDescriptorHandleForHeapStart();
		UINT samplerDescriptorSize = pID3DDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);

		for (int i = 0; i < 2; ++i)
		{
			pID3DDevice->CreateSampler(&samplerDescs[i], samplerHandle);
		...
		TCHAR pszShaderFileName[] = _T("Shader/Texture.hlsl");
		...
		GRS_THROW_IF_FAILED(pID3DDevice->CreatePlacedResource(pIUploadHeap.Get()
			...
			, D3D12_RESOURCE_STATE_GENERIC_READ
			, nullptr
			, IID_PPV_ARGS(&pITextureUpload1)));


		n64BufferOffset += GRS_UPPER(BUFFER_SIZE * BUFFER_SIZE * 4, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);
		...
		GRS_THROW_IF_FAILED(pID3DDevice->CreatePlacedResource(pIUploadHeap.Get()
			...
			, D3D12_RESOURCE_STATE_GENERIC_READ
			, nullptr
			, IID_PPV_ARGS(&pITextureUpload2)));

		n64BufferOffset += GRS_UPPER(BUFFER_SIZE * BUFFER_SIZE * 4, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);

		LoadTexture2D(pID3DDevice, _T("Texture/bear.jpg"), pIBMP, stTextureFormat, nTextureW, nTextureH, nBPP, nPicRowPitch, pITextureHeap, pITexcute1, pITextureUpload1, pIUploadHeap, pICommandQueue, pICommandList, pICommandAllocator, pIPipelineState, pIFence);

		LoadTexture2D(pID3DDevice, _T("Texture/bear_flipX.jpg"), pIBMP, stTextureFormat, nTextureW, nTextureH, nBPP, nPicRowPitch, pITextureHeap, pITexcute2, pITextureUpload2, pIUploadHeap, pICommandQueue, pICommandList, pICommandAllocator, pIPipelineState, pIFence);
		...
		//	, D3D12_RESOURCE_STATE_COPY_DEST
		//	, nullptr
		//	, IID_PPV_ARGS(&pITexcute2)));
		////-----------------------------------------------------------------------------------------------------------

		////获取上传堆资源缓冲的大小，这个尺寸通常大于实际图片的尺寸
		const UINT64 n64UploadBufferSize = GetRequiredIntermediateSize(pITexcute1.Get(), 0, 1);
		...
		//	D3D12_HEAP_DESC stUploadHeapDesc = {  };
		//	//尺寸依然是实际纹理数据大小的2倍并64K边界对齐大小
		//	stUploadHeapDesc.SizeInBytes = GRS_UPPER(n64UploadBufferSize * 10, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);
		//	//注意上传堆肯定是Buffer类型，可以不指定对齐方式，其默认是64k边界对齐
		//	stUploadHeapDesc.Alignment = 0;
		//	stUploadHeapDesc.Properties.Type = D3D12_HEAP_TYPE_UPLOAD;		//上传堆类型
		//	stUploadHeapDesc.Properties.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_UNKNOWN;
		...
		//n64BufferOffset += GRS_UPPER(n64UploadBufferSize, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);
		...
		//D3D12_RESOURCE_DESC stDestDesc = pITexcute1->GetDesc();
		...
		////向命令队列发出从上传堆复制纹理数据到默认堆的命令
		//CD3DX12_TEXTURE_COPY_LOCATION Dst1(pITexcute1.Get(), 0);
		//CD3DX12_TEXTURE_COPY_LOCATION Src1(pITextureUpload.Get(), stTxtLayouts);
		//pICommandList->CopyTextureRegion(&Dst1, 0, 0, 0, &Src1, nullptr);
		...
		//stResBar.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		//stResBar.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
		//stResBar.Transition.pResource = pITexcute1.Get();
		//stResBar.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
		//stResBar.Transition.StateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
		//stResBar.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		...
		//CD3DX12_TEXTURE_COPY_LOCATION Dst2(pITexcute2.Get(), 0);
		//CD3DX12_TEXTURE_COPY_LOCATION Src2(pITextureUpload.Get(), stTxtLayouts);
		//pICommandList->CopyTextureRegion(&Dst2, 0, 0, 0, &Src2, nullptr);

		////设置一个资源屏障，同步并确认复制操作完成
		////直接使用结构体然后调用的形式
		//stResBar.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		//stResBar.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
		//stResBar.Transition.pResource = pITexcute2.Get();
		//stResBar.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
		//stResBar.Transition.StateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
		//stResBar.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		...
		stSRVDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
		stSRVDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
		stSRVDesc.Texture2D.MipLevels = 1;

		nSRVDescriptorSize = pID3DDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

		CD3DX12_CPU_DESCRIPTOR_HANDLE stSRVHandle(pISRVHeap->GetCPUDescriptorHandleForHeapStart());

		pID3DDevice->CreateShaderResourceView(pITexcute1.Get(), &stSRVDesc, stSRVHandle);

		stSRVHandle.Offset(1, nSRVDescriptorSize);

		pID3DDevice->CreateShaderResourceView(pITexcute2.Get(), &stSRVDesc, stSRVHandle);
		...
		while (!bExit)
		{
			...
			ID3D12DescriptorHeap* ppHeaps[] = { pISRVHeap.Get(), samplerHeap.Get() };
			...
			pICommandList->SetGraphicsRootDescriptorTable(1, samplerHeap->GetGPUDescriptorHandleForHeapStart());

			CD3DX12_GPU_DESCRIPTOR_HANDLE stGPUCBVHandle(pISRVHeap->GetGPUDescriptorHandleForHeapStart()
				, 1
				, pID3DDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
			//设置CBV
			pICommandList->SetGraphicsRootDescriptorTable(2, stGPUCBVHandle);
			...
			// 继续记录命令，并真正开始新一帧的渲染
			const float clearColor[] = { 100 / 255.0f, 149 / 255.0f, 237 / 255.0f, 1.0f };
