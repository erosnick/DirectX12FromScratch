...
struct Vertex2D
{
	glm::vec2 m_vPos;		//Position
	glm::vec2 m_vTex;		//Texcoord
	glm::vec4 m_vColor;		//Color
};

struct VertexQuad
{
	glm::vec4 m_vPos;		//Position
	glm::vec4 m_vColor;		//Color
	glm::vec2 m_vTex;		//Texcoord
};
...
{
	glm::mat4 m_MVO;	//经典的Model-view-projection(MVP)矩阵.
};
...
public:
	...
	void createDescriptorHeap(ComPtr<ID3D12DescriptorHeap>& descriptorHeap, UINT numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, D3D12_DESCRIPTOR_HEAP_FLAGS flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE);

	template<class T>
	void createVertexBuffer(const std::vector<T>& stTriangleVertices, ComPtr<ID3D12Resource>& buffer);
	...
	UINT32 createIndexBuffer(const std::vector<UINT32>& indices, ComPtr<ID3D12Resource>& indexBuffer, D3D12_INDEX_BUFFER_VIEW& indexBufferView);
	...
	std::vector<UINT> mSphereIndices;
...
void RenderToTexture::createDescriptorHeap(ComPtr<ID3D12DescriptorHeap>& descriptorHeap, UINT numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type, D3D12_DESCRIPTOR_HEAP_FLAGS flags) 
{
...
template<class T>
void RenderToTexture::createVertexBuffer(const std::vector<T>& stTriangleVertices, ComPtr<ID3D12Resource>& buffer)
{
	UINT bufferSize = stTriangleVertices.size() * sizeof(T);
	...
	//计算边界对齐的正确的偏移位置
	mBufferOffset = GRS_UPPER(mBufferOffset + bufferSize, D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);
}
...
UINT32 RenderToTexture::createIndexBuffer(const std::vector<UINT32>& indices, ComPtr<ID3D12Resource>& indexBuffer, D3D12_INDEX_BUFFER_VIEW& indexBufferView)
{
	const UINT nszIndexBuffer = indices.size() * sizeof(UINT32);
 ...
 void RenderToTexture::createPipelineStateObject(const std::vector<D3D12_INPUT_ELEMENT_DESC>& inputElementDescs, const ComPtr<ID3D12RootSignature>&	rootSignature, const ShaderData& shader, ComPtr<ID3D12PipelineState>& pipelineState, bool wireframe, bool depth, bool ccw, D3D12_PRIMITIVE_TOPOLOGY_TYPE primitiveTopologyType)
{
    ...
    stPSODesc.InputLayout = {inputElementDescs.data(), (UINT)inputElementDescs.size()};
...
void RenderToTexture::resetUploadCommandList()
{
	...
	ThrowIfFailed(mUploadCommandList->Reset(mDirectCmdListAlloc.Get(), nullptr));
	//---------------------------------------------------------------------------------------------
}
...
void RenderToTexture::createCommandAllocators()
{
	...
	, mDirectCmdListAlloc.Get(), nullptr, IID_PPV_ARGS(&mUploadCommandList)));
}
...
void RenderToTexture::createRootSignature()
{
	...
	CD3DX12_ROOT_PARAMETER1 stRootParameters[5];
	...
	stRootParameters[4].InitAsConstantBufferView(2, 0, D3D12_ROOT_DESCRIPTOR_FLAG_DATA_VOLATILE, D3D12_SHADER_VISIBILITY_ALL);		//CBV是所有Shader可见
	...
	// 创建渲染矩形的根签名对象
	D3D12_DESCRIPTOR_RANGE1 stDSPRangesQuad[3];
	stDSPRangesQuad[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	stDSPRangesQuad[0].NumDescriptors = 1;
	stDSPRangesQuad[0].BaseShaderRegister = 0;
	stDSPRangesQuad[0].RegisterSpace = 0;
	stDSPRangesQuad[0].Flags = D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE;
	stDSPRangesQuad[0].OffsetInDescriptorsFromTableStart = 0;

	stDSPRangesQuad[1].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
	stDSPRangesQuad[1].NumDescriptors = 1;
	stDSPRangesQuad[1].BaseShaderRegister = 0;
	stDSPRangesQuad[1].RegisterSpace = 0;
	stDSPRangesQuad[1].Flags = D3D12_DESCRIPTOR_RANGE_FLAG_NONE;
	stDSPRangesQuad[1].OffsetInDescriptorsFromTableStart = 0;

	stDSPRangesQuad[2].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER;
	stDSPRangesQuad[2].NumDescriptors = 1;
	stDSPRangesQuad[2].BaseShaderRegister = 0;
	stDSPRangesQuad[2].RegisterSpace = 0;
	stDSPRangesQuad[2].Flags = D3D12_DESCRIPTOR_RANGE_FLAG_NONE;
	stDSPRangesQuad[2].OffsetInDescriptorsFromTableStart = 0;

	D3D12_ROOT_PARAMETER1 stRootParametersQuad[3];
	stRootParametersQuad[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	stRootParametersQuad[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
	...
	stRootParametersQuad[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	stRootParametersQuad[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	stRootParametersQuad[1].DescriptorTable.NumDescriptorRanges = 1;
	stRootParametersQuad[1].DescriptorTable.pDescriptorRanges = &stDSPRangesQuad[1];

	stRootParametersQuad[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	stRootParametersQuad[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
	stRootParametersQuad[2].DescriptorTable.NumDescriptorRanges = 1;
	stRootParametersQuad[2].DescriptorTable.pDescriptorRanges = &stDSPRangesQuad[2];
...
void RenderToTexture::createBindlessResources()
{
	...
	g_d3d.cbv_srv_uav.Init(g_d3d.device, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 5, true,  L"CBV SRV UAV Heap");
...
{
    // 定义正方形的3D数据结构
    std::vector<Vertex> stTriangleVertices =
	{
	...
	createVertexBuffer(stTriangleVertices, mVertexBuffer, mVertexBufferView);
}

void RenderToTexture::createIndexBuffer()
{
	std::vector<UINT32> pBoxIndices //立方体索引数组
		= {
		0,1,2,
		3,4,5,
	...
	mIndexCount = createIndexBuffer(pBoxIndices, mIndexBuffer, mIndexBufferView);
}
...
void RenderToTexture::createEarthVertexBuffer()
{
	//22、加载球体的网格数据
	{
		...
		mSphereVertices.resize(mSphereVertexCnt);
		mSphereIndices.resize(mSphereVertexCnt);
	...
	createVertexBuffer(mSphereVertices, mVertexBufferEarth, mVertexBufferViewEarth);
}

void RenderToTexture::createEarthIndexBuffer()
{
	createIndexBuffer(mSphereIndices, mIndexBufferEarth, mIndexBufferViewEarth);
}
...
void RenderToTexture::createQuadVertexBuffer()
{
	...
	mQuadVertexCnt = stTriangleVertices.size();
}
...
void RenderToTexture::recordBundles()
{
	...
	//Draw Call！！！
	mBundlesSkybox->DrawInstanced(mSkyboxVertices.size(), 1, 0, 0);
	...
	mBundlesQuad->SetGraphicsRootDescriptorTable(1, stGPUCBVHandleQuad);
	mBundlesQuad->SetGraphicsRootDescriptorTable(2, mDHSampleQuad->GetGPUDescriptorHandleForHeapStart());
...
{
	// 1. 上下翻转（Y 取反）
	mxOrthographic = glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, -1.0f, 1.0f));

	// 2. 微量偏移，矫正像素位置
	mxOrthographic = glm::translate(mxOrthographic, glm::vec3(-0.5f, +0.5f, 0.0f)) * mxOrthographic;
	...
	glm::mat4 ortho = glm::ortho(left, right, bottom, top, znear, zfar);
 	...
 	mxOrthographic = glm::translate(mxOrthographic, glm::vec3(160.0f, 120.0f, 0.0f));

	// 将正交矩阵乘到当前矩阵
	mxOrthographic = ortho * mxOrthographic;

	mxOrthographic = glm::scale(mxOrthographic, glm::vec3(160.0f, 120.0f, 1.0f));
...
void RenderToTexture::loadDDSTexture(const std::wstring& path, ComPtr<ID3D12Resource>& texture)
{
	...
	uint64_t uploadBufferSkyboxSize = GetRequiredIntermediateSize(texture.Get(), 0, static_cast<UINT>(subresources.size()));
...
void RenderToTexture::createShaderResourceView()
{
	// Earth
	// 2 SRV + 1 CBV
	createDescriptorHeap(mSRVHeap, 3, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE);
...
void RenderToTexture::createQuadResources()
{
	...
	cbvDesc.SizeInBytes = static_cast<UINT>(mMVOBufferSize);
	...
	const UINT texWidth  = (uint32_t)texDesc.Width;
	const UINT texHeight = (uint32_t)texDesc.Height;
	std::vector<UINT32> test(texWidth * texHeight);
...
void RenderToTexture::Update(const GameTimer& gt)
{
	...
	auto view = glm::lookAtLH(glm::vec3(0.0f, 10.0f, -20.0f), glm::vec3(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
{
	{
		...
		mCommandList->SetGraphicsRoot32BitConstants(3, 1, &objectIndex, 0);
...

	{
		...
		mCommandList->ExecuteBundle(mBundlesQuad.Get());
		...
		mCommandList->SetGraphicsRootDescriptorTable(1, stGPUCBVHandleQuad);
		mCommandList->SetGraphicsRootDescriptorTable(2, mDHSampleQuad->GetGPUDescriptorHandleForHeapStart());
