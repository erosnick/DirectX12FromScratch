...
void CreateDXGI(UINT nDXGIFactoryFlags, ComPtr<IDXGIFactory5>& pIDXGIFactory5, HWND hWnd)
{
	// 创建DXGI Factory对象
...
void EnableDebugLayer(UINT nDXGIFactoryFlags)
{
	// 打开显示子系统的调试支持
#if defined(_DEBUG)
...
void CreateDevice(ComPtr<IDXGIFactory5> pIDXGIFactory5, ComPtr<IDXGIAdapter1>& pIAdapter1, D3D_FEATURE_LEVEL emFeatureLevel, ComPtr<ID3D12Device4>& pID3D12Device4, HWND hWnd)
{
...
void CreateCommandQueue(ComPtr<ID3D12Device4> pID3D12Device4, ComPtr<ID3D12CommandQueue>& pICMDQueue)
{
	// 创建直接命令队列
...
void CreateSwapChain(const UINT nFrameBackBufCount, int iWidth, int iHeight, DXGI_FORMAT emRenderTargetFormat, ComPtr<IDXGIFactory5> pIDXGIFactory5, ComPtr<ID3D12CommandQueue>& pICMDQueue, HWND hWnd, ComPtr<IDXGISwapChain1>& pISwapChain1, ComPtr<IDXGISwapChain3>& pISwapChain3)
{
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		EnableDebugLayer(nDXGIFactoryFlags);

		CreateDXGI(nDXGIFactoryFlags, pIDXGIFactory5, hWnd);

		CreateDevice(pIDXGIFactory5, pIAdapter1, emFeatureLevel, pID3D12Device4, hWnd);

		CreateCommandQueue(pID3D12Device4, pICMDQueue);

		// 创建交换链
		{
			CreateSwapChain(nFrameBackBufCount, iWidth, iHeight, emRenderTargetFormat, pIDXGIFactory5, pICMDQueue, hWnd, pISwapChain1, pISwapChain3);
...
#endif
			GRS_THROW_IF_FAILED(D3DCompileFromFile(_T("Shader/shaders.hlsl")
				, nullptr
				, nullptr
				, "VSMain"
				, "vs_5_0"
		...
		while (!bExit)
		{
			...
			case 0:
			{
				...
				GRS_THROW_IF_FAILED(pICommandList->Reset(pICMDAlloc.Get(), pIPipelineState.Get()));
