// 2-D3D12WICTexture.cpp : Defines the entry point for the application.
//

#include "framework.h"
#include "2-D3D12WICTexture.h"
...
DXGI_FORMAT LoadTexture2D(const WCHAR* pszTexcuteFileName, ComPtr<IWICBitmapSource>& pIBMP, DXGI_FORMAT& stTextureFormat, UINT& nTextureW, UINT& nTextureH, UINT& nBPP, UINT& nPicRowPitch)
{
	...
	nPicRowPitch = (uint64_t(nTextureW) * uint64_t(nBPP) + 7u) / 8u;		return stTextureFormat;
}
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		const UINT64 n64UploadBufferSize = GetRequiredIntermediateSize(pITexcute1.Get(), 0, 1) * 2;
